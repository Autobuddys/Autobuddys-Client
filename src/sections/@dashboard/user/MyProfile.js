...
const MyProfile = () => {
    ...
    useEffect(()=>{
      ...
      async function getData() {
        await axiosInstance.post('elder/relpat_data/',{"patID":localStorage.getItem("patientID"),"relID":obj.id})
        .then((res)=>{
    ...
    const handleSubmitRel=(e)=>{
      e.preventDefault();
      console.log(obj.id)
      axiosInstance.patch(`elder/profile/${obj.id}/`,{name:relnameRef.current.value,phone:relphoneRef.current.value,email:relmailRef.current.value})
        .then((res)=>{
            location.reload()
        })
        .catch((err)=>{
    ...
    const handleSubmit=(e)=>{
      ...
      axiosInstance.put(`elder/patient/${localStorage.getItem("patientID")}/`,{patrel:obj['id'],pname:nameRef.current.value,pphone:phoneRef.current.value,page:ageRef.current.value,address:addressRef.current.value,city:cityRef.current.value,state:stateRef.current.value,pincode:pinRef.current.value,dname:dnameRef.current.value,dphone:dphoneRef.current.value})
        .then((res)=>{
            // console.log(res)
            location.reload()
        })
